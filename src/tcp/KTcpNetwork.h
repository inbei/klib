...
namespace klib {
    ...
    public:
        KTcpNetwork()
            :KEventObject<SocketType>("Poll thread"),m_connected(false), 
        ...
        virtual bool Start(const std::string& ip, int32_t port, bool isServer = true, bool needAuth = false)
        {
            ...
            m_isServer = isServer;
            m_needAuth = needAuth;
            return KEventObject<SocketType>::Start();
        }
    ...
    private:
            ...
            else
            {
                if (m_connected)
                {
                    if (PollSocket() < 1)
                    {
                        ...
                        if (ReadSocket(m_fd, bufs) < 0)
                        {
                            DisconnectConnection(m_fd);
                        ...
                        {
                            SendDataToConnection(m_fd, bufs);
                        }
                    }
                }
                else
        ...
        void CreateConnection(SocketType fd, const std::string& ipport)
        {
            ...
            std::map<SocketType, KTcpConnection<MessageType>*>::iterator it = m_connections.begin();
            ...
            else
            {
                if (m_connections.size() < m_maxClient)
                {
                    ...
                    if (c->Start(m_isServer ? NmServer : NmClient, ipport, m_needAuth))
                    {
                        m_connections[fd] = c;
                    }
                    else
        ...
        void SendReadEventToConnection(SocketType fd)
        {
            std::map<SocketType, KTcpConnection<MessageType>*>::iterator it = m_connections.find(fd);
            if (it != m_connections.end())
            {
                KTcpConnection<MessageType>* c = it->second;
                SocketEvent e;
                e.fd = fd;
                e.ev = SocketEvent::SeRecv;
                if (c->IsConnected() && !c->Post(e))
                    printf("Send event to connection failed, fd:[%d]\n", fd);
            }
        }
        ...
        void SendDataToConnection(SocketType fd, const std::vector<KBuffer>& bufs)
        {
            std::map<SocketType, KTcpConnection<MessageType>*>::iterator it = m_connections.find(fd);
            if (it != m_connections.end())
            {
