...
#define WriteFile(file, format, timestamp) \
    {\
        int tsz = 0;\
        int wsz = 0;\
        if (timestamp)\
        {\
            std::string datetime;\
            KTime::NowDateTime("yyyy-mm-dd hh:nn:ss.ccc", datetime);\
            wsz = fprintf(file, "%s  ", datetime.c_str()); \
            if (wsz > 0)\
            {\
                tsz += wsz; \
            }\
...
#define MakeMessage(fmt, buf, rsz) \
    ...
    {
    public:
        KTextFile(size_t maxsize = 50/*mb*/)
            :m_file(NULL), m_hexBuffer(HexBufferSize),
            m_maxSize(maxsize * 1024 * 1024),
            m_totalSize(0)
        {
        ...
        virtual void Initial(const std::string& path, const std::string& filename, bool timestamp = false)
        {
            m_path = path;
            m_timestamp = timestamp;
            m_fileName = filename;
            m_filePath = path + PathSeparator + filename;
        }
        ...
        size_t WriteString(const char* format, ...)
        {
            ...
            WriteFile(m_file, format, m_timestamp);
        }
    ...
    */
    ...
    {
    public:
        virtual void Initial(const std::string& path, const std::string& filename, bool timestamp)
        {
            KLockGuard<KMutex> lock(m_fileMutex);
            KTextFile::Initial(path, filename, timestamp);
        }
    ...
    */
    ...
    public:
        ...
        bool Initial(const std::string& path, const std::string& filename, bool timestamp)
        {
            m_timestamp = timestamp;
            m_file.Initial(path, filename, false);
        ...
        {
            Close();
            m_file.Remove(m_file.GetFilePathName());
        }
