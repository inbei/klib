...
namespace thirdparty {
	...
	std::string KInfluxDbCli::DumpToInfluxdbPoint(const std::string& tablename, const std::map<std::string, std::string>& tags, const std::map<std::string, std::string>& numfields, const std::map<std::string, std::string>& strfields)
	{
	...
	bool KInfluxDbCli::CreateDB(const std::string& database)
	{
		std::string resp;
		return Post("http://" + m_host + ":8086/query?u=" + m_username + "&p=" + m_password, std::string("q=create database ") + database, resp);
	}

	bool KInfluxDbCli::CreateRetentionPolicy(const std::string& retentionpolicy, const std::string& duration1, const std::string& sharedduration, bool bdefault /*= false*/)
	{
	...
	bool KInfluxDbCli::CreateContinuousQuery(const std::string& cqname, const std::string& interval, const std::map<std::string, std::string>& aggfuncalias, const std::vector<std::string> groupbyfields)
	{
		std::string resp;
	...
	bool KInfluxDbCli::InsertPoint(const std::vector<std::string>& sqls)
	{
	...
	bool KInfluxDbCli::QueryMesurement(const std::string& sql, std::string& tablename, QuaryResult& qr)
	{
		std::string resp;
	...
	void KInfluxDbCli::SetEasyOpt(const std::string& url, CURL* curl, std::string* resp) const
	{
		// set opts  
	...
	bool KInfluxDbCli::ParseInfluxdbResponse(const std::string& resp, std::string& tablename, QuaryResult& qr)
	{
	...
	size_t KInfluxDbCli::WriteCallback(void* data, size_t size, size_t nmemb, void* buffer)
	{
	...
	bool KInfluxDbCli::Post(const std::string& url, const std::string& dat, std::string& resp)
	{
		long rc = 0;
		try
		{
	...
	void KInfluxDbCli::SetRetentionPolicy(const std::string& retentionpolicy)
	{
