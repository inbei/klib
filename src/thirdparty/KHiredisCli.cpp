...
namespace thirdparty {
    ...
    bool KHiredisCli::Select(uint16_t index)
    {
    ...
    bool KHiredisCli::Flushdb()
    {
        KLockGuard<KMutex> lock(m_redisMutex);
        return Exec(m_redisContext, "flushdb");
    }

    bool KHiredisCli::Remove(const std::string& pattern)
    {
    ...
    bool KHiredisCli::Sadd(const std::string& key, const std::set<std::string>& elements)
    {
        std::string cmd("sadd ");
        cmd.append(key);
    ...
    bool KHiredisCli::Smembers(const std::string& key, std::vector<std::string>& vals)
    {
    ...
    bool KHiredisCli::PipelineCmd(const std::vector<std::string>& cmds, std::vector<redisReply*>& replies)
    {
        KLockGuard<KMutex> lock(m_redisMutex);
        if (m_redisContext)
        {
    ...
    bool KHiredisCli::PipelineCmd(const std::vector<RedisArgvCmd>& cmds, std::vector<redisReply*>& replies)
    {
        KLockGuard<KMutex> lock(m_redisMutex);
        if (m_redisContext)
        {
    ...
    bool KHiredisCli::Mset(const std::map<std::string, std::string>& keyvals)
    {
    ...
    bool KHiredisCli::Mget(const std::vector<std::string>& kys, std::vector<std::string>& vals)
    {
    ...
    bool KHiredisCli::Hmset(const std::string& key, const std::map<std::string, std::string>& fieldValues)
    {
    ...
    bool KHiredisCli::Hmget(const std::string& key, std::vector<std::string>& fields, std::vector<std::string>& values)
    {
        if (fields.empty())
        {
    ...
    int KHiredisCli::HgetAll(const std::string& key, std::vector<std::string>& fields, std::vector<std::string>& values)
    {
    ...
    redisContext* KHiredisCli::Connect(RedisConfig& conf)
    {
    ...
    bool KHiredisCli::Exec(redisContext* ctx, const std::string& cmd)
    {
        if (ctx)
        {
    ...
    int KHiredisCli::Remove(const std::vector<std::string>& kys)
    {
    ...
    int KHiredisCli::MgetInternal(const std::vector<std::string>& kys, std::vector<std::string>& vals)
    {
        std::ostringstream os;
        os << "mget";
        std::vector<std::string>::const_iterator it = kys.begin();
    ...
    int KHiredisCli::HmgetInternal(const std::string& key, const std::vector<std::string>& kys, std::vector<std::string>& vals)
    {
        std::ostringstream os;
        os << "hmget " << key;
        std::vector<std::string>::const_iterator it = kys.begin();
    ...
    int KHiredisCli::MsetInternal(std::map<std::string, std::string>::const_iterator first, std::map<std::string, std::string>::const_iterator last)
    {
    ...
    int KHiredisCli::HmsetInternal(const std::string& key, std::map<std::string, std::string>::const_iterator first, std::map<std::string, std::string>::const_iterator last)
    {
    ...
    int KHiredisCli::GetResponse(redisReply* reply, std::string& val)
    {
        if (!reply)
        {
            Close();
    ...
    int KHiredisCli::GetResponse(redisReply* reply, std::vector<std::string>& vals)
    {
        if (!reply)
        {
            Close();
